...
class ps_users(db.Model):
    ...
    u_status = db.Column(db.Enum(UserStatus), nullable=False, default=UserStatus.ACTIVE)
    u_first_login = db.Column(db.Boolean(), server_default=expression.true(), nullable=False)
    u_account_type = db.Column(db.Enum('particulier', 'supermarche', 'pharmacie', 'quincaillerie', 'autre'), default='particulier')
    u_business_name = db.Column(db.String(255))
    u_business_address = db.Column(db.Text())
    u_business_location = db.Column(db.String(255))
    u_phone = db.Column(db.String(128))
    creation_date = db.Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow)
    updated_on = db.Column(db.DateTime, nullable=False, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow)
